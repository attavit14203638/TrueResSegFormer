...
"""
Centralized pipeline for TCD-SegFormer.

This module provides unified entry points for training, evaluation, and prediction
of the TCD-SegFormer model, consolidating functionality that was previously spread
...
def run_training_pipeline(
    ...
    """
    Run the complete TCD-SegFormer training pipeline.
    ...
    model, optimizer, scheduler = create_model(
        ...
        apply_loss_at_original_resolution=config.get("apply_loss_at_original_resolution", True),
    ...
    # Save model using the HuggingFace save_pretrained method
            ...
            if amp_enabled and scaler is not None:
                ...
                if (step + 1) % gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                    ...
                    if global_step > 0 and global_step % eval_steps == 0:
                        ...
                        if save_best:
                            save_checkpoint(model=model, optimizer=optimizer, scheduler=scheduler, global_step=global_step, epoch=epoch,
                                            output_dir=output_dir, metrics=metrics, metric_to_monitor=metric_to_monitor,
                                            monitor_mode=monitor_mode, max_to_keep=max_to_keep)
                        model.train()

                    # Save model checkpoint (periodic save, MOVED INSIDE OPTIMIZER STEP)
                    if global_step > 0 and global_step % save_steps == 0:
                        save_checkpoint(model=model, optimizer=optimizer, scheduler=scheduler, global_step=global_step, epoch=epoch,
                                        output_dir=output_dir, max_to_keep=max_to_keep)
...
def run_prediction_pipeline(
    ...
    """
    Run the complete TCD-SegFormer prediction pipeline.
    ...
    # Load id2label mapping from the actually loaded model's config
    ...
    for idx, (image_pil, image_np) in enumerate(zip(images, image_nps)):
        ...
        if show_class_activation_maps and hasattr(model, 'segformer'):
            ...
            try:
                ...
                # Generate CAM (function now handles saving internally if save_path is provided)
                cam_vis_array = plot_class_activation_map(
                    model=model,
                    image=image_pil, # Pass original PIL Image
                    image_processor=image_processor,
                    device=device,
                    save_path=cam_path # Pass save path directly
                    # target_layer can be omitted to use default
                    # target_class defaults to 1 (tree_crown)
                )
