...
def save_checkpoint(
    ...
    try:
        ...
        # Save training arguments (if applicable, assuming args is accessible or passed in)
        # Example: You might want to save hparams or config here
        # config_path = os.path.join(checkpoint_dir, 'training_config.json')
        # with open(config_path, 'w') as f: 
        #    json.dump(vars(args), f, indent=2) # Assuming 'args' holds your config
...
def verify_checkpoint(checkpoint_dir: str) -> bool:
    ...
    has_model_file = any(os.path.exists(os.path.join(checkpoint_dir, f)) for f in model_files)
    
    if not has_model_file:
        logging.warning(f"No model file found in {checkpoint_dir}")
        return False
    
    # Check for required files
    for file in required_files:
        if not os.path.exists(os.path.join(checkpoint_dir, file)):
            logging.warning(f"Required file {file} not found in {checkpoint_dir}")
            return False
...
def load_model_for_evaluation(
    ...
    try:
        ...
        if is_local_path:
            ...
            if not is_true_res:
                ...
                try:
                    ...
                    hf_backbone_name = config.get('model_name') # Get HF backbone identifier, e.g., "nvidia/mit-b5"
        ...
        # Create appropriate image processor
        from transformers import SegformerImageProcessor
        image_processor = SegformerImageProcessor(
            do_resize=False,  # Keep original image size
            do_rescale=True,  # Rescale to [0,1]
            do_normalize=True  # Standard normalization
        )
